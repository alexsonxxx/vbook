...
function execute(url, page) {
    ...
    // /models/story (category/tag browse) paginates qua pageSize; bootstrap chỉ trả list cố định.
    var sizeParam = url.indexOf("/models/story") >= 0 ? "pageSize" : "limit";
    var apiPath = url + sep + "page=" + p + "&" + sizeParam + "=" + PAGE_SIZE;
    ...
    var hasNext = false;
    if (json.pagination && json.pagination.totalPages != null) {
        // /models/story pagination: totalPages tin cậy, hasMore luôn false → dựa vào tổng số trang.
        hasNext = p < json.pagination.totalPages;
    }
    // Bootstrap (home/ranking) trả list cố định, mỗi requested page giống nhau → không phân trang.
